...
// Backward-compatibility alias: some UIs call /upload-notification
router.post('/:jobId/upload-notification', authenticate, uploadAny.single('notice'), async (req, res) => {
  // Delegate to the same logic as /upload-notice by reusing implementation
  ...
  try {
    ...
    console.error('upload-notification aliasda xatolik:', err);
    return res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// Tekshiruvchi/Yurist: ishni tugallangan deb belgilash (guvohnoma bor, jarayon tugadi)
router.post('/:id/mark-completed', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Noto‘g‘ri ID' });
    const job = await Job.findById(id);
    ...
    job.status = 'finished';
    job.history = job.history || [];
    job.history.push({ status: 'finished', comment: 'Ish tugallandi', date: new Date(), updatedBy: req.user?.id });
    job.updatedAt = new Date();
    await job.save();
    // Bildirishnomalar
    try {
      if (job.assignedTo) await Notification.create({ user: job.assignedTo, job: job._id, type: 'job_completed', title: 'Ish tugallandi', message: job.brandName || '', link: `/jobs/${job._id}` });
      await Notification.create({ role: 'admin', job: job._id, type: 'job_completed', title: 'Ish tugallandi', message: job.brandName || '', link: `/jobs/${job._id}` });
    } catch (e) { console.warn('Notif error:', e.message); }
    res.json({ message: 'Ish tugallandi', status: job.status });
  } catch (err) {
    console.error('mark-completed da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// Admin: yakuniy holatga o‘tkazish (finalized)
router.post('/:id/finalize', authenticate, async (req, res) => {
  try {
    ...
    job.history = job.history || [];
    job.history.push({ status: 'finalized', comment: 'Yakuniy holat', date: new Date(), updatedBy: req.user?.id });
    job.updatedAt = new Date();
    await job.save();
    res.json({ message: 'Yakuniy holatga o\'tkazildi', status: job.status });
  } catch (err) {
    console.error('finalize da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// Admin: arxivga o‘tkazish
router.post('/:id/archive', authenticate, async (req, res) => {
  try {
    ...
    job.history = job.history || [];
    job.history.push({ status: 'archived', comment: 'Arxivga o\'tkazildi', date: new Date(), updatedBy: req.user?.id });
    await job.save();
    res.json({ message: 'Arxivga o\'tkazildi', archived: true });
  } catch (err) {
    console.error('archive da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// Admin: set payouts for a job
router.post('/:id/payouts', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Noto‘g‘ri ID' });
    const job = await Job.findById(id);
    ...
    const { operator, tekshiruvchi, yurist, currency, note } = req.body || {};
    const payouts = job.payouts || {};
    payouts.operator = { ...(payouts.operator || {}), amount: Number(operator?.amount || 0), approved: Boolean(operator?.approved) };
    payouts.tekshiruvchi = { ...(payouts.tekshiruvchi || {}), amount: Number(tekshiruvchi?.amount || 0), approved: Boolean(tekshiruvchi?.approved) };
    payouts.yurist = { ...(payouts.yurist || {}), amount: Number(yurist?.amount || 0), approved: Boolean(yurist?.approved) };
    payouts.currency = currency || payouts.currency || 'UZS';
    payouts.note = note || payouts.note || '';
    payouts.total = Number((payouts.operator?.amount || 0) + (payouts.tekshiruvchi?.amount || 0) + (payouts.yurist?.amount || 0));
    payouts.calculatedAt = new Date();
    payouts.calculatedBy = req.user?.id;
    job.payouts = payouts;

    job.history = job.history || [];
    job.history.push({ status: job.status, comment: `Ish haqi hisoblandi: ${payouts.total} ${payouts.currency}`, date: new Date(), updatedBy: req.user?.id });
    await job.save();
    res.json({ success: true, payouts: job.payouts });
  } catch (err) {
    console.error('payouts set da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});
...
router.get('/:id/payouts', authenticate, async (req, res) => {
  try {
    ...
    const job = await Job.findById(id);
    if (!job) return res.status(404).json({ message: 'Ish topilmadi' });
    res.json({ payouts: job.payouts || null });
  } catch (err) {
    console.error('payouts get da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// Certificate download by job id (operator/admin/lawyer)
router.get('/:id/certificate', authenticate, async (req, res) => {
  try {
    ...
    const job = await Job.findById(id);
    if (!job) return res.status(404).json({ message: 'Ish topilmadi' });
    const cert = job.documents?.find(d => d.type === 'certificate') || (job.certificatePath ? { path: job.certificatePath } : null);
    if (!cert || !cert.path) return res.status(404).json({ message: 'Guvohnoma topilmadi' });
    const filePath = path.isAbsolute(cert.path) ? cert.path : path.join(__dirname, '..', cert.path);
    if (!fs.existsSync(filePath)) return res.status(404).json({ message: 'Guvohnoma fayli yo‘q' });
    const name = `guvohnoma_${job.clientName || 'mijoz'}_${job.brandName || ''}${path.extname(filePath)}`;
    res.download(filePath, name);
  } catch (err) {
    console.error('certificate download da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});

// List documents for a job (for admin visibility)
router.get('/:id/documents', authenticate, async (req, res) => {
  try {
    ...
    const job = await Job.findById(id);
    if (!job) return res.status(404).json({ message: 'Ish topilmadi' });
    res.json({ documents: job.documents || [], certificatePath: job.certificatePath || null });
  } catch (err) {
    console.error('documents list da xatolik:', err);
    res.status(500).json({ message: 'Server xatosi', error: err.message });
  }
});
