...
function ExcelImport({ token, onImportComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await apiFetch('/api/jobs/import-excel', {
        method: 'POST',
        body: formData
      });
  ...
  const handleDownloadTemplate = () => {
    // Namuna template faylni yaratish va yuklab olish logikasi
    // Bu yerda faqat namuna URL orqali havola qo'shilgan
    alert('Namuna fayl yuklanmoqda...');
  };
  ...
  return (
    <div className="excel-import-container">
      ...
      <div className="excel-template">
        ...
        <button className="download-template" onClick={handleDownloadTemplate}>Namuna faylni yuklab olish</button>
      </div>
    </div>
  );
}
