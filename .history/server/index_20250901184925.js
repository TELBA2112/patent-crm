...
const registerRoutes = () => {
  ...
  try {
    ...
    // Vaqtinchalik oddiy job-actions API endpoint yaratamiz
    app.post('/api/job-actions/:jobId/send-for-review', (req, res) => {
      res.status(501).json({ 
        message: 'Bu funksiya server muammosi sababli hozircha mavjud emas', 
        error: 'jobActions moduli yuklanmadi'
      });
    });
    
    app.post('/api/job-actions/:jobId/submit-documents', (req, res) => {
      res.status(501).json({ 
        message: 'Bu funksiya server muammosi sababli hozircha mavjud emas', 
        error: 'jobActions moduli yuklanmadi'
      });
    ...
    console.log('⚠️ Vaqtinchalik job-actions endpointlari yaratildi (501 xatolik qaytaradi)');
  }
};
