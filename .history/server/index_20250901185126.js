...
const registerRoutes = () => {
  ...
  try {
    ...
    app.post('/api/jobs/:jobId/power-of-attorney', (req, res) => {
      try {
        const { jobId } = req.params;
        console.log(`[ISHONCHNOMA] Job ${jobId} uchun ishonchnoma saqlandi`);
        res.status(200).json({ message: 'Ishonchnoma muvaffaqiyatli saqlandi' });
      } catch (err) {
    ...
    app.get('/api/jobs/:jobId/power-of-attorney-pdf', (req, res) => {
      try {
        // Oddiy PDF fayl yaratish o'rniga, HTML formatni qaytaramiz
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename="ishonchnoma.pdf"');
        
        // Oddiy PDF fayl yaratish - haqiqiy holatda PDF generatsiya qilish kerak
        const fs = require('fs');
        ...
        if (fs.existsSync(samplePdfPath)) {
          ...
          res.status(404).json({ message: 'PDF fayl topilmadi' });
        }
      } catch (err) {
