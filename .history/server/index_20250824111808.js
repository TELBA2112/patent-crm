...
// Debug middleware - har bir so'rovni kuzatish
app.use((req, res, next) => {
  console.log(`[${new Date().toISOString()}] ${req.method} ${req.url}`);
...
// Ro'yxatdan o'tgan routerlar tekshiruvi
app._router.stack.forEach(layer => {
  if (layer.route) {
    console.log(`Route: ${layer.route.path}, Methods: ${Object.keys(layer.route.methods)}`);
  } else if (layer.name === 'router' && layer.handle.stack) {
    console.log(`Router base path: ${layer.regexp}`);
    layer.handle.stack.forEach(routerLayer => {
      if (routerLayer.route) {
        console.log(`  Sub-route: ${routerLayer.route.path}, Methods: ${Object.keys(routerLayer.route.methods)}`);
      }
    });
  }
});
