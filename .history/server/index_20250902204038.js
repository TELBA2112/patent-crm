...
// Statik yuklangan fayllarni servis qilish (uploads/)
...
// Minimal yurist statistikasi (klient ehtiyoji uchun)
app.get('/api/statistics/lawyer', async (req, res) => {
  try {
    const Job = require('./models/Job');
    const toLawyer = await Job.countDocuments({ status: 'to_lawyer' });
    const processing = await Job.countDocuments({ status: 'lawyer_processing' });
    const completed = await Job.countDocuments({ status: 'lawyer_completed' });
    res.json({ toLawyer, processing, completed });
  } catch (e) {
    res.status(500).json({ message: 'Statistika xatosi', error: e.message });
  }
});
