...
// Improved route registration with error handling
const registerRoutes = () => {
  ...
  try {
    ...
    console.error('❌ Users routes registration error:', err);
  }
  ...
  try {
    ...
    console.error('❌ Jobs routes registration error:', err);
  }

  try {
    // Try to load the standard jobActions routes
    ...
    console.error('❌ JobActions routes registration error:', err);
    console.log('⚠️ Trying to load simplified jobActions routes as a fallback...');
    
    try {
      // If the main jobActions fails, try to load the simplified version as a fallback
      const jobActionsSimple = require('./routes/jobActions-simple');
      app.use('/api/job-actions', jobActionsSimple);
      console.log('✅ Simplified /api/job-actions routeri ro\'yxatdan o\'tkazildi');
    } catch (fallbackErr) {
      console.error('❌ Even simplified JobActions routes failed:', fallbackErr);
    }
  }
};
