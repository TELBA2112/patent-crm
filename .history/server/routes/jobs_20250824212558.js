...
router.post('/:id/upload-docs', authenticate, async (req, res) => {
  try {
    ...
    if (personType === 'yuridik' && yuridikDocs) {
      ...
      return res.status(400).json({ message: 'Noto\'g\'ri hujjat turi' });
    }

    await job.save();
    console.log('MUVAFFAQIYAT: Hujjatlar saqlandi');
    res.json({ message: 'Hujjatlar muvaffaqiyatli saqlandi', jobId: id });
  } catch (err) {
    console.error('XATOLIK: Hujjatlarni saqlashda muammo:', err.message);
    console.error(err.stack);
    res.status(500).json({ message: 'Serverda xatolik', error: err.message });
  }
});
