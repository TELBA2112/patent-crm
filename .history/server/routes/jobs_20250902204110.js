...
router.get('/', async (req, res, next) => {
  try {
  const { status, assignedTo, search, fromDate, toDate } = req.query;
  const filter = {};
    if (status) {
  ...
  if (search) {
    ...
    if (fromDate || toDate) {
      ...
      if (toDate) {
        ...
        if (!isNaN(to.getTime())) filter.createdAt.$lte = new Date(to.getTime() + 24*60*60*1000 - 1);
      }
...
router.post('/import-excel', authenticate, uploadMemory.single('excelFile'), async (req, res, next) => {
  try {
...
// Guvohnomalarni (.rar va boshqa) yuklash - fayllarni uploads/ ichiga saqlash
router.post('/:id/upload-certificates', authenticate, uploadDisk.array('certificates', 5), async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Noto‘g‘ri ID' });
    const job = await Job.findById(id);
    ...
    await job.save();
    res.json({ success: true, certificates: job.certificates });
  } catch (err) { next(err); }
});
