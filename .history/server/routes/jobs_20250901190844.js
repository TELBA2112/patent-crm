...
const User = mongoose.model('User'); // User modeli import qilingan
const auth = require('../middleware/auth');
const multer = require('multer'); // Fayllarni yuklash uchun
const fs = require('fs'); // Fayl tizimi bilan ishlash uchun
const xlsx = require('xlsx'); // Excel fayllarni o'qish uchun
const path = require('path'); // Fayl yo'llarini boshqarish uchun

// Fayllarni yuklash uchun konfiguratsiya
const upload = multer({
  dest: 'uploads/',
  limits: { fileSize: 10 * 1024 * 1024 }, // 10MB maksimal fayl hajmi
});

// Admin ekanini tekshirish uchun middleware
const isAdmin = (req, res, next) => {
  if (req.user && req.user.role === 'admin') {
    next();
  } else {
    res.status(403).json({ message: 'Faqat administratorlar bu amaliyotni bajarishi mumkin' });
  }
};

// Job obyektini to'ldirish uchun yordamchi funksiya
const populateJob = async (jobQuery) => {
