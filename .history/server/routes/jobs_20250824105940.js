...
// POST /api/jobs — Yangi ish yaratish (faqat admin)
router.post('/', authenticate, async (req, res) => {
  console.log('POST /api/jobs so\'rovi qabul qilindi');
  console.log('Foydalanuvchi roli:', req.user?.role);
  console.log('So\'rov tanasi:', req.body);
  
  try {
    if (req.user.role !== 'admin') {
      console.log('Foydalanuvchi admin emas. Ruxsat rad etildi.');
      return res.status(403).json({ message: 'Faqat admin yangi ish yaratishi mumkin' });
    }
    ...
    if (missingFields.length > 0) {
      console.log('Quyidagi maydonlar to\'ldirilmagan:', missingFields);
      return res.status(400).json({ message: `Quyidagi maydonlar to'ldirilmagan: ${missingFields.join(', ')}` });
    }
    ...
    const newJob = new Job({
      ...
      status: 'yangi',
      history: [{
        action: 'Admin tomonidan ish qo\'shildi',
        status: 'yangi',
    ...
    await newJob.save();
    console.log('Yangi ish yaratildi:', newJob._id);
...
router.patch('/:id', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Admin harakatlari
      ...
      // Admin barcha maydonlarni o'zgartira oladi
      const { clientName, clientSurname, phone, brandName, personType, assignedTo, tekshiruvchiId, yuristId, status, comments } = req.body;

      if (clientName) job.clientName = clientName;
      if (clientSurname) job.clientSurname = clientSurname;
      if (phone) job.phone = phone;
      if (brandName) job.brandName = brandName;
      if (personType) job.personType = personType;
      if (assignedTo) job.assignedTo = assignedTo;
      if (tekshiruvchiId) job.tekshiruvchi = tekshiruvchiId;
      if (yuristId) job.yurist = yuristId;
