...
// Save power of attorney for a job
router.post('/:id/save-power-of-attorney', authenticate, async (req, res) => {
  try {
    const jobId = req.params.id;
    const { content, personType, classes } = req.body;
    
    console.log('Received save-power-of-attorney request:', {
      jobId,
      personType,
    ...
    // Save the document to the job
    if (!job.documents) {
      job.documents = [];
    }
    
    // Check if any power of attorney already exists and update it
    const poaIndex = job.documents.findIndex(doc => doc.type === 'power-of-attorney');
    
    const powerOfAttorney = {
      type: 'power-of-attorney',
      content: content,
      createdAt: new Date(),
      personType: personType || 'jismoniy'
    };
    
    if (poaIndex !== -1) {
      job.documents[poaIndex] = powerOfAttorney;
    } else {
      job.documents.push(powerOfAttorney);
    }
    
    // Make sure the job has classes property correctly set
    if (classes && Array.isArray(classes)) {
      // Process the classes to ensure they're consistent
      const processedClasses = classes.map(classItem => {
        if (typeof classItem === 'object' && classItem.classNumber) {
          return classItem.classNumber;
        }
        // Try to convert to number if possible
        const classNum = Number(classItem);
        return !isNaN(classNum) ? classNum : classItem;
      });
      
      // Update the job with processed class numbers
      job.classes = processedClasses;
      console.log('Updated job classes:', job.classes);
    }
    
    await job.save();
    
    res.json({ 
      message: 'Power of attorney saved successfully',
      document: powerOfAttorney,
      classes: job.classes // Return the saved classes for confirmation
    });
  } catch (err) {
