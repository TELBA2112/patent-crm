...
const Job = mongoose.model('Job'); // Model already registered in ./models

// GET /api/jobs — list with optional filters
router.get('/', async (req, res, next) => {
  try {
    const { assignedTo, tekshiruvchi, yurist, search, status } = req.query;
    const query = {};

    if (assignedTo) query.assignedTo = assignedTo;
    if (tekshiruvchi) query.tekshiruvchi = tekshiruvchi;
    if (yurist) query.yurist = yurist;
