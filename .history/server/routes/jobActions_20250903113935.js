...
router.post('/:jobId/invoices/:invoiceId/upload-receipt', authenticate, upload.single('receipt'), async (req, res) => {
  try {
...
router.post('/:id/complete-by-lawyer', authenticate, upload.single('certificates'), async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ message: 'Noto‘g‘ri ID' });
    const job = await Job.findById(id);
