...
router.post('/:id/upload-docs', authenticate, async (req, res) => {
  try {
    ...
    // Foydalanuvchi ruxsatini tekshirish - operator va admin ruxsat oladi
    ...
    } else if (personType === 'jismoniy' && jismoniyDocs) {    res.status(500).json({ message: 'Server xatoligi', error: err.message });
...
module.exports = router;});  }    res.status(500).json({ message: 'Hujjatlarni saqlashda server xatosi', error: err.message });    console.error('Hujjatlarni saqlashda xatolik:', err);  } catch (err) {    });      job      message: 'Hujjatlar muvaffaqiyatli saqlandi',    res.json({        console.log(`${id} ID uchun hujjatlar muvaffaqiyatli saqlandi`);    await job.save();        });      date: new Date()      updatedBy: req.user.id,      status: 'documents_submitted',      action: 'Hujjatlar yuklandi va tekshiruvchiga yuborildi',    job.history.push({    // Tarixga yozib qo'yish        job.status = 'documents_submitted';    // Status o'zgartirish - hujjatlar yuborilgan statusga o'tkazish        }});
