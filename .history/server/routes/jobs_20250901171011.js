...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // For non-admins, only show jobs assigned to them or relevant to their role
    ...
    const jobs = await Job.find(filter).populate('assignedTo', 'firstName lastName username');
    res.json(jobs);
  } catch (err) {
...
router.get('/:id', auth, async (req, res) => {
  try {
    const job = await Job.findById(req.params.id).populate('assignedTo tekshiruvchi yurist', 'firstName lastName username');
    if (!job) {
...
// POST /api/jobs/import-excel - Import jobs from Excel
...
router.post('/:id/update-mktu-classes', auth, async (req, res) => {
  try {
    ...
    if (req.body.classes && Array.isArray(req.body.classes)) {
      ...
      const normalizedClasses = req.body.classes
      ...
      console.log('Jobs route: MKTU sinflari muvaffaqiyatli yangilandi:', normalizedClasses);
      return res.json({ 
        success: true, 
