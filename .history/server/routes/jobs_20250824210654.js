...
// POST /api/jobs/:id/upload-docs - Hujjatlarni saqlash va tekshiruvchiga yuborish
router.post('/:id/upload-docs', authenticate, async (req, res) => {
  try {
    ...
    console.log('Hujjatlar yuklash boshlandi:', { id, personType });
    ...
    // Faqat tasdiqlangan brendlar uchun yoki qaytarilgan hujjatlar uchun
    if (job.status !== 'approved' && job.status !== 'documents_pending' && job.status !== 'documents_returned') {
      return res.status(400).json({ message: 'Bu ish hujjat yuborish holatida emas' });
    }
    
    console.log('Hujjatlarni saqlash:', personType);
