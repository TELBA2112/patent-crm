...
// Ishonchnomani saqlash uchun yangi route
router.post('/:jobId/save-power-of-attorney', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
    const { content, personType } = req.body;
    
    if (!content) {
      return res.status(400).json({ message: "Ishonchnoma kontenti kerak" });
    }
    ...
    // Hujjatlar massivini yaratish yoki mavjudiga qo'shish
    if (!job.documents) {
      job.documents = [];
    }
    
    // Ishonchnomani qo'shish
    job.documents.push({
    ...
    // Ish tarixiga qo'shish
    job.history.push({
      action: 'power-of-attorney-created',
      user: req.user.id,
      date: new Date(),
      details: `${personType} shaxs uchun ishonchnoma yaratildi`
    });
    ...
    res.status(200).json({
      ...
      documentId: job.documents[job.documents.length - 1]._id
    });
  } catch (error) {
