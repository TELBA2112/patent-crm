...
router.get('/stats', auth, async (req, res) => {  // Changed authenticate to auth
  try {
    const stats = {
...
router.post('/:id/send-for-review', auth, async (req, res) => {  // Changed authenticate to auth
  const { id } = req.params;
...
router.post('/:id/review-brand', auth, async (req, res) => {
  const { id } = req.params;
...
router.post('/:id/send-to-lawyer', auth, async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/:id/accept-by-lawyer', auth, async (req, res) => {
  const { id } = req.params;
  const { comment } = req.body;
  const userId = req.user.id;
...
router.post('/:id/complete-by-lawyer', auth, async (req, res) => {
  const { id } = req.params;
  const { comment } = req.body;
  const userId = req.user.id;
...
router.post('/:id/return-documents', auth, async (req, res) => {
  try {
    const { id } = req.params;
...
// POST /api/job-actions/:id/save-power-of-attorney - Ishonchnomani saqlash
router.post('/:id/save-power-of-attorney', auth, async (req, res) => {
  try {
    ...
    const normalizedClasses = Array.isArray(classes) 
      ? classes.map(cls => parseInt(cls)).filter(c => !isNaN(c))
      : [];
    ...
    // Also update the main classes field on the job
    job.classes = normalizedClasses;

    job.history.push({
      action: 'Ishonchnoma yaratildi va saqlandi',
    ...
    res.status(200).json({ message: 'Ishonchnoma muvaffaqiyatli saqlandi', job });
...
// GET /api/job-actions/:id/power-of-attorney-pdf - PDF ishonchnoma generatsiya qilish
router.get('/:id/power-of-attorney-pdf', auth, async (req, res) => {
  try {
    const jobId = req.params.id;
    const job = await Job.findById(jobId);
