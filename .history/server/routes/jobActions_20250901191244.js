...
router.post('/:jobId/send-for-review', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
    const { brandName, classes, activityNotes } = req.body;
    
    console.log(`[BREND YUBORISH] So'rov ${jobId} uchun qabul qilindi:`, { brandName, activityNotes });
    ...
    // Brendni, faoliyat izohlarini va sinflarni saqlash
    job.brandName = brandName;
    job.activityNotes = activityNotes || ''; // Faoliyat turi haqidagi izohni saqlash
    ...
    // Tarix qo'shish
    job.history.push({
      action: `Brend "${brandName}" tekshiruvga yuborildi`,
    ...
    return res.status(200).json({ 
      ...
      activityNotes: job.activityNotes,
      classes: job.classes,
      status: job.status
    });
  } catch (err) {
...
router.post('/:jobId/review-brand', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
    ...
    if (approved === true) {
      ...
      // Tarixga yozish
      job.history.push({
        action: `Brend "${job.brandName}" tekshiruvchidan tasdiqlandi`,
        status: 'approved',
        updatedBy: req.user.id,
        date: new Date()
      });
      ...
      // Tarixga yozish
      job.history.push({
        action: `Brend "${job.brandName}" tekshiruvchidan rad etildi. Sabab: ${reason || 'Ko\'rsatilmagan'}`,
...
router.post('/:jobId/submit-documents', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
    const { personType, classes, yuridikDocs, jismoniyDocs } = req.body;
    
    console.log(`[HUJJATLAR] So'rov ${jobId} uchun qabul qilindi. Shaxs turi: ${personType}`);
    ...
    // Tarixga yozish
    job.history.push({
      action: `${personType === 'yuridik' ? 'Yuridik' : 'Jismoniy'} shaxs hujjatlari yuborildi`,
...
router.get('/:jobId/power-of-attorney', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
