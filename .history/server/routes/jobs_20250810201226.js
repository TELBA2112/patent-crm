...
// jobs.js (backend)
router.put('/:id/send-to-review', authenticate, async (req, res) => {
  const { id } = req.params;
  const { brandName, brandLogo } = req.body; // Tekshiruvchining ID'si ham kelishi mumkin
  const userId = req.user.id;
  const userRole = req.user.role;

  try {
    // Faqat operatorlar bu ishni qila olsin
    if (userRole !== 'operator') {
      return res.status(403).json({ message: 'Faqat operatorlar bu harakatni amalga oshira oladi' });
    }
    ...
    job.status = 'in_review'; // 'brand_sent' yoki 'in_review' statuslardan birini tanlang.
    
    // Tekshiruvchini tayinlash (agar tekshiruvchi ID si kelgan bo'lsa)
    // Bu yerda tekshiruvchining ID'sini o'zingiz aniqlaysiz.
    // Masalan, barcha tekshiruvchilardan birini tasodifiy tanlash mumkin.
    // Hozircha uni bo'sh qoldirib turamiz yoki frontenddan kelgan ID'ni ishlatamiz
    // job.tekshiruvchi = req.body.tekshiruvchiId;

    await job.save();

    res.json({ message: 'Brend muvaffaqiyatli tekshiruvchiga yuborildi', job });

  } catch (err) {
    console.error('Brendni tekshiruvchiga yuborishda xatolik:', err);
