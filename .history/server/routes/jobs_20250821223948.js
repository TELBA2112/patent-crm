...
router.get('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (role === 'operator') {
      ...
      // Tekshiruvchi o'ziga yuborilgan va qaytarilgan ishlarni ko'rishi kerak
      filter.$or = [{ status: 'brand_in_review' }, { status: 'returned_to_inspector' }];
    } else if (role === 'yurist') {
...
router.patch('/:id', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (role === 'operator') {
        ...
        // Operator tomonidan brendni tekshiruvga yuborish
      ...
      // Admin uchun har qanday statusni o'zgartirish
      if(status) {
        job.status = status;
        job.comments = comments;
      ...
      if(req.body.tekshiruvchi) job.tekshiruvchi = req.body.tekshiruvchi;
      if(req.body.yurist) job.yurist = req.body.yurist;
    }
...
router.post('/:id/comment', authenticate, async (req, res) => {
    ...
    try {
      ...
      if (role !== 'yurist' || job.yurist?.toString() !== userId) {
...
// DELETE: Ishni o‘chirish (faqat admin)
