...
exports.authenticate = (req, res, next) => {
  ...
  console.log('Sorov:', req.method, req.url);
  
  try {
    const authHeader = req.headers.authorization;
    if (!authHeader) {
      console.log('XATOLIK: Authorization header mavjud emas');
      return res.status(401).json({ message: 'Autentifikatsiya tokeni yoq' });
    }
    
    if (!authHeader.startsWith('Bearer ')) {
      console.log('XATOLIK: Notogri token formati');
      return res.status(401).json({ message: 'Notogri token formati' });
    }
    ...
    return res.status(401).json({ message: 'Notogri yoki eskirgan token' });
  }
};
