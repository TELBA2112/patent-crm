...
const startServer = async () => {
  try {
    ...
    if (isAvailable) {
      // Port bo'sh, serverni ishga tushirish
      app.listen(PORT, () => {
        console.log(`\n✅ Server ${PORT} portda ishlayapti`);
        console.log(`📡 API manzili: http://localhost:${PORT}`);
      });
    } else {
      // Port band, qanday qilib bo'shatish kerakligi haqida ko'rsatmalar
      console.error(`\n⚠️ ${PORT} port band!`);
      console.log('\nPortni bo\'shatish uchun quyidagi amallarni bajaring:');
      console.log(`1. sudo lsof -i :${PORT} - portni ishlatayotgan jarayonni topish`);
      console.log('2. sudo kill -9 <PID> - jarayonni to\'xtatish');
      console.log('Yoki boshqa port tanlang: .env faylida PORT=5001 deb yozing');
      process.exit(1);
    }
  } catch (err) {
    console.error('MongoDB ga ulanishda xatolik:', err);
    console.error('Iltimos, MongoDB server ishlab turganini tekshiring.');
    process.exit(1);
  }
};
