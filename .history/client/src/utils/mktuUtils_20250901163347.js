...
/**
 * MKTU sinflarini serverga saqlash - with failover mechanism
 * @param {string} jobId - Job ID
...
export const saveJobMktuClasses = async (jobId, classes, token) => {
  try {
    ...
    // First try the job-actions endpoint
    try {
      ...
      if (res.ok) {
        console.log('MKTU classes saved successfully via job-actions endpoint');
        return true;
      }
      
      console.warn('Failed to save via job-actions endpoint, trying fallback...');
    } catch (error) {
      console.warn('Error with job-actions endpoint:', error);
    }
    
    // Fallback to the jobs endpoint
    ...
    if (!fallbackRes.ok) {
      const errorData = await fallbackRes.json().catch(() => ({ message: 'Unknown error' }));
      throw new Error(`Failed to save MKTU classes: ${errorData.message}`);
    }
    
    console.log('MKTU classes saved successfully via fallback endpoint');
    return true;
  } catch (error) {
