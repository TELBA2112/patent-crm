...
// Store selected MKTU classes in localStorage and try to save to the server
export const storeMktuClasses = async (classes, jobId, token) => {
  ...
  console.log("Storing MKTU classes:", classes);
  
  // First save to localStorage as a backup
  localStorage.setItem('selectedMktuClasses', JSON.stringify(classes));
  
  // Try to save to the server if jobId and token are provided
  if (jobId && token) {
    try {
      const response = await fetch(`http://localhost:5000/api/jobs/${jobId}/mktu-classes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify({ classes: classes.map(c => parseInt(c)) })
      });
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`Error saving MKTU classes to server: ${errorText}`);
      } else {
        console.log("Successfully saved MKTU classes to server");
      }
    } catch (error) {
      console.error("Error saving MKTU classes to server:", error);
    }
  }
...
// Get selected MKTU classes from server or localStorage as fallback
export const getStoredMktuClasses = async (jobId, token) => {
  // Try to get from server first if jobId and token are provided
  if (jobId && token) {
    try {
      const response = await fetch(`http://localhost:5000/api/jobs/${jobId}/mktu-classes`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.classes && Array.isArray(data.classes) && data.classes.length > 0) {
          console.log("Retrieved MKTU classes from server:", data.classes);
          return data.classes;
        }
      }
    } catch (error) {
      console.error("Error retrieving MKTU classes from server:", error);
    }
  }
  
  // Fall back to localStorage if server request fails or is not attempted
  try {
