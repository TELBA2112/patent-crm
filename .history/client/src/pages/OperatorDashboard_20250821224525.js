...
function OperatorDashboard({ onLogout }) {
  ...
  const [tekshiruvchilar, setTekshiruvchilar] = useState([]); // Yangi: Tekshiruvchilar ro'yxati
  const [tekshiruvchiId, setTekshiruvchiId] = useState(''); // Yangi: Tanlangan tekshiruvchi ID si
  ...
  // Yangi funksiya: Tekshiruvchilar ro'yxatini olish
  const fetchTekshiruvchilar = useCallback(async () => {
    try {
      ...
      alert('Tekshiruvchilarni olishda xatolik: ' + err.message);
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    // Yangi: Dashbord yuklanganda tekshiruvchilar ro'yxatini olish
    if (selectedSection === 'yangi') {
      fetchTekshiruvchilar();
    }
  }, [selectedSection, fetchProfile, fetchTasks, fetchTekshiruvchilar]);
  ...
  const handleTaskClick = (task) => {
    ...
    setBrandName('');
    setBrandLogo('');
    setTekshiruvchiId(''); // Holatni tozalash
    setPersonType('');
    setDocsStep(1);
    setYuridikDocs({});
    setJismoniyDocs({});
      ...
      case 3:
        return (
          <div className="step-box">
            ...
            <p>Mijozning brend nomini kiriting va tekshiruvchini tanlang.</p>
            <input
              type="text"
            ...
            {tekshiruvchilar.length > 0 && (
              <select
                value={tekshiruvchiId}
                onChange={e => setTekshiruvchiId(e.target.value)}
                className="select-tekshiruvchi"
              >
            ...
            <button onClick={sendBrandForReview} className="btn-primary" disabled={!brandName || !tekshiruvchiId || loading}>Tekshiruvchiga yuborish</button>
          </div>
        );
      case 4:
        return (
