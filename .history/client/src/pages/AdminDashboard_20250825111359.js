...
function AdminDashboard({ onLogout }) {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      console.log('Ishlarni olishga urinish');
      ...
      if (!res.ok) {
        ...
        try {
          ...
          throw new Error(`Ishlarni olishda xatolik: ${errorText}`);
        }
      }
      
      // Tasks komponenti o'zi ma'lumotlarni olishi mumkin
      // Bu funksiya asosan import qilingandan so'ng qayta yuklash uchun
    } catch (err) {
  ...
  const handleImportComplete = (results) => {
    if (results && results.success > 0) {
      // Ishlar ro'yxatini yangilash
      if (current === 'ishlar') {
        fetchJobs(); // Now fetchJobs is defined
      }
    }
  };
