...
function OperatorDashboard({ onLogout }) {
  ...
  // MKTU sinflar tanlovi rendereri - faqat qidiruv orqali ko'rsatiladigan versiya
  const renderClassSelection = () => {
    return (
      <div className="class-selection">
                ...
                {recommendedClasses.map(recClass => (
                  <div key={recClass.classNumber} className="recommended-class-item">
                    ...
                    <div className="rec-class-info">
                      ...
                      <p>{recClass.description.substring(0, 150)}...</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Qisqa raqamlar ro'yxati */}
        <div className="quick-numbers-grid">
          <h4>Tezkor raqamlar:</h4>
          <div className="numbers-grid">
            {Array.from({ length: 45 }, (_, i) => i + 1).map(num => (
              <div 
                key={num}
                className={`number-item ${selectedClasses.includes(num) ? 'selected' : ''}`}
                onClick={() => handleClassToggle(num)}
                title={mktuClasses.find(c => c.classNumber === num)?.name || `Sinf ${num}`}
              >
                {num}
              </div>
            ))}
...
<style jsx>{`
  ...
  /* Tezkor raqamlar stiklarini yangilash */
  .quick-numbers-grid {
    margin: 20px 0;
    padding: 15px;
    background: #fff;
