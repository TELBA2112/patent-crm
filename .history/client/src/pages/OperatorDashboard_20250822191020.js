...
function OperatorDashboard({ onLogout }) {
  const [selectedSection, setSelectedSection] = useState('yangi'); // Tanlangan bo‘lim (yangi, jarayonda, tugatilgan)
  const [profile, setProfile] = useState(null); // Foydalanuvchi profili
  const [loading, setLoading] = useState(false); // Yuklanish holati
  const [tasks, setTasks] = useState([]); // Ishlar ro‘yxati
  const [tasksLoading, setTasksLoading] = useState(false); // Ishlar yuklanishi
  const [selectedTask, setSelectedTask] = useState(null); // Tanlangan ish
  const [showModal, setShowModal] = useState(false); // Modal oynasi ko‘rinishi
  const [step, setStep] = useState(1); // Modal ichidagi qadam
  const [showPhone, setShowPhone] = useState(false); // Telefon raqamini ko‘rsatish
  const [phoneTimer, setPhoneTimer] = useState(10); // Telefon raqami ko‘rinish vaqti (10 sekund)
  const [callResult, setCallResult] = useState(''); // Qo‘ng‘iroq natijasi
  const [clientIntent, setClientIntent] = useState(''); // Mijozning niyati
  const [futureDate, setFutureDate] = useState(''); // Kelajakdagi sana (keyinroq uchun)
  const [brandName, setBrandName] = useState(''); // Brend nomi
  const [personType, setPersonType] = useState(''); // Shaxs turi (yuridik/jismoniy)
  const [yuridikDocs, setYuridikDocs] = useState({}); // Yuridik hujjatlar
  const [jismoniyDocs, setJismoniyDocs] = useState({}); // Jismoniy hujjatlar
  const [docsStep, setDocsStep] = useState(1); // Hujjat yig‘ish qadami
  const token = localStorage.getItem('token'); // Autentifikatsiya tokeni
  ...
  const sendBrandForReview = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Brendni yuborishda xatolik');
  ...
  // Hujjat yig‘ish qadamlarini ko‘rsatish (placeholder, chunki to‘liq kod yo‘q)
