...
function OperatorDashboard({ onLogout }) {
  ...
  const handleTaskClick = (task) => {
    ...
    // MKTU sinflarini to'g'ri formatta olish
    if (task.classes && Array.isArray(task.classes)) {
      // Classes ni aniq raqamlarga aylantirish
      const classNumbers = task.classes.map(cls => {
        if (typeof cls === 'object' && cls.classNumber) {
          return cls.classNumber;
        }
        return Number(cls) || cls; // Convert to number if possible
      });
  ...
  // Hujjatlarni saqlash va tekshiruvchiga yuborish - MKTU sinflarini qo'shish bilan
  const saveDocsAndSendToReview = async () => {
    ...
    try {
      ...
      // MKTU sinflari tekshirilishi va qo'shilishi
      let classesToSend = [];
      if (selectedClasses && selectedClasses.length > 0) {
        classesToSend = selectedClasses;
      } else if (selectedTask.classes && selectedTask.classes.length > 0) {
        classesToSend = selectedTask.classes;
      }
      
      console.log('Yuborilayotgan ma\'lumotlar:', {
        personType,
        classes: classesToSend, // MKTU sinflarini ham yuborish
        ...docsData
      });
      ...
      const res = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/upload-docs`, {
        ...
        body: JSON.stringify({
          personType,
          classes: classesToSend, // MKTU sinflarini yuborish
          ...docsData
        })
      });
  ...
  // Hujjatlar yig'ish qadamlarini ko'rsatish - yangilangan MKTU sinflari ko'rinishi bilan
  const renderDocsStep = () => {
    ...
    return (
      <div className="docs-collection-form">
        ...
        {/* MKTU sinflarini ko'rsatish bo'limi */}
        <div className="selected-mktu-classes-section">
          ...
          {selectedClasses && selectedClasses.length > 0 ? (
            ...
            <p className="no-mktu-selected">MKTU sinflari tanlanmagan</p>
          )}
  ...
  // MKTU sinf tanlovi rendereri - tanlangan sinar ko'rinadigan qilib yangilangan
  const renderClassSelection = () => {
    return (
      <div className="class-selection">
        ...
        {/* Tanlangan sinflarni ko'rsatish - har doim ko'rinishi uchun shart olib tashlandi */}
...
<style jsx>{`
  ...
  /* Qo'lda sinf qo'shish uchun stillar */
  .add-class-manually {
    margin-top: 20px;
    padding: 15px;
  ...
  .add-class-manually h4 {
    margin-top: 0;
    font-size: 16px;
    margin-bottom: 10px;
  }
  
  .manual-class-input select {
    width: 100%;
    padding: 12px;
    border-radius: 8px;
  ...
  .manual-class-input select:focus {
    border-color: #4CAF50;
    outline: none;
    box-shadow: 0 0 0 2px rgba(76, 175, 80, 0.2);
  }
  
  /* Tanlangan sinf itemlari uchun yorqinroq dizayn */
  .selected-class-item {
    display: flex;
    align-items: center;
  ...
  /* MKTU ko'rinishi uchun yangi stillar */
  .mktu-classes-display {
    margin: 20px 0;
    padding: 15px;
    background: #f5f8ff;
    border-radius: 8px;
  ...
  .selected-mktu-classes-section h4 {
    margin-top: 0;
    color: #2962ff;
    font-size: 16px;
    margin-bottom: 12px;
