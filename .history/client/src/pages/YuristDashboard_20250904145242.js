...
function YuristDashboard({ onLogout }) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setError('');
    try {
      // Use role-specific views for clearer grouping
      let url;
      if (selectedSection === 'yangi') {
        url = `${API_BASE}/api/jobs/views/lawyer/new`;
      } else if (selectedSection === 'jarayonda') {
        url = `${API_BASE}/api/jobs/views/lawyer/in-progress`;
      } else if (selectedSection === 'tugatilgan') {
        url = `${API_BASE}/api/jobs/views/lawyer/completed`;
      } else {
