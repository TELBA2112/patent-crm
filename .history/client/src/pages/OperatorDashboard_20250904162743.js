...
// Helper function to determine if a task has been processed by the reviewer and sent to the lawyer
const isTaskCompleted = (task) => {
  if (!task) return false;
  
  // Task statuses that indicate the task has moved beyond operator editing
...
function OperatorDashboard({ onLogout }) {
  ...
  const sendBrandForReview = async () => {
    ...
    // Check if the task status allows operator actions
    if (isTaskCompleted(selectedTask)) {
      alert('Bu ish tekshiruvchi tomonidan yuristga yuborilgan. Endi uni tahrirlash mumkin emas.');
      return;
    }
  ...
  const handleTaskClick = (task) => {
    ...
    // Tekshirish - yuristga yuborilgan yoki yurist bilan ishlash bosqichidagi ishlar tahrirlash uchun o'chirilgan bo'lishi kerak
    const lawyerStatuses = ['to_lawyer', 'lawyer_processing', 'lawyer_completed', 'finished', 'finalized'];
    if (lawyerStatuses.includes(task.status)) {
      setStep(5); // Read-only bosqichi (yangi maxsus bosqich)
      console.log('Yuristga yuborilgan yoki yurist tomonidan bajarilgan ish - faqat ko\'rish rejimida');
      return;
    }
    ...
    if (task.status === 'approved' || task.status === 'documents_pending' || 
      ...
      // Avoid resetting to step 1 if job has progressed beyond initial contact
      const advancedStatuses = ['brand_in_review'];
      if (advancedStatuses.includes(task.status)) {
        setStep(3);
        console.log('Oldinga o\'tgan ish, MKTU/hujjatlar bosqichi');
      } else {
        setStep(1);
  ...
  // Faqat ko'rish rejimidagi ishni ko'rsatish - yuristga yuborilgan ishlar uchun
  const renderReadOnlyJobView = () => {
    return (
      <div className="step-box read-only-mode">
        <h3>Ushbu ish tekshiruvchi tomonidan yuristga yuborilgan</h3>
        <div className="read-only-info">
          <p className="warning-text">⚠️ Yuristga yuborilgan ishlar operatorlar tomonidan tahrirlana olmaydi.</p>
          
          <div className="job-details-readonly">
            <h4>Ish ma'lumotlari:</h4>
            <table className="readonly-table">
              <tbody>
                <tr>
                  <th>Ish ID:</th>
                  <td>{selectedTask?.jobId || selectedTask?._id}</td>
                </tr>
                <tr>
                  <th>Mijoz:</th>
                  <td>{selectedTask?.clientName} {selectedTask?.clientSurname}</td>
                </tr>
                <tr>
                  <th>Telefon:</th>
                  <td>{selectedTask?.phone}</td>
                </tr>
                <tr>
                  <th>Brend:</th>
                  <td>{selectedTask?.brandName}</td>
                </tr>
                <tr>
                  <th>Status:</th>
                  <td>
                    <span className={`status-tag status-${selectedTask?.status}`}>
                      {selectedTask?.status === 'to_lawyer' ? 'Yuristga yuborilgan' :
                       selectedTask?.status === 'lawyer_processing' ? 'Yurist qayta ishlayotgan' :
                       selectedTask?.status === 'lawyer_completed' ? 'Yurist yakunlagan' :
                       selectedTask?.status === 'finished' ? 'Tugatilgan' : selectedTask?.status}
                    </span>
                  </td>
                </tr>
              </tbody>
            ...
            {/* Guvohnomalarni ko'rsatish - agar mavjud bo'lsa */}
            {selectedTask?.certificates && selectedTask.certificates.length > 0 && (
              <div className="certificates-section">
                <h4>Guvohnomalar:</h4>
                <ul className="certificates-list">
                  {selectedTask.certificates.map((cert, idx) => (
                    <li key={idx}>
                      <a href={`${API_BASE}/${cert.path.replace(/^\//, '')}`} 
                         target="_blank" 
                         rel="noreferrer" 
                         className="certificate-link">
                        Guvohnoma #{idx + 1} {cert.uploadedAt ? `(${new Date(cert.uploadedAt).toLocaleDateString()})` : ''}
                      </a>
                    </li>
                  ))}
                </ul>
  ...
  const renderModalContent = () => {
    ...
    switch (step) {
      case 5: // Read-only mode for lawyer-related tasks
        return renderReadOnlyJobView();
      case 1:
        return (
        ...
        <div className="task-cards-modern">
          {tasks.map(task => (
            <div key={task._id} 
              ...
              {(task.status === 'lawyer_completed' || task.status === 'finished' || task.status === 'finalized') && (
                <p>
                  <a
                    href={`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/job-actions/${task._id}/certificate`}
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            {Array.isArray(selectedTask.history) && selectedTask.history.length > 0 && (
              <div className="history-wrapper" style={{margin:'10px 0'}}>
                <h4>Tarix</h4>
                <HistoryBlock history={selectedTask.history} />
              </div>
            )}
