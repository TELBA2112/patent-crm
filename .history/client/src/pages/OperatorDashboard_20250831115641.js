...
import { mktuClasses, getRecommendedClasses, storeMktuClasses, getStoredMktuClasses, sharePowerOfAttorneyDocuments } from '../utils/mktuData';
...
function OperatorDashboard({ onLogout }) {
      ...
      case 4:
        return (
          <div className="step-box">
            ...
            {/* Add JSHSHIR input for jismoniy shaxs when creating power of attorney */}
            {personType === 'jismoniy' && (
              <div className="jshshir-input-container">
                <label>JSHSHIR (ishonchnoma uchun):</label>
                <input
                  type="text"
                  value={jshshrInput}
                  onChange={handleJshshrInputChange}
                  placeholder="JSHSHIR raqamini kiriting"
                  className="jshshir-input"
                />
              </div>
            )}
            
            {/* Show power of attorney buttons if the task has personType */}
            {selectedTask && selectedTask.personType && (
              <div className="power-of-attorney-section">
                <h4>Ishonchnoma yaratish</h4>
                {renderPowerOfAttorneyButtons(selectedTask)}
              </div>
            )}
  ...
  // Check for stored MKTU classes on component mount
  useEffect(() => {
    // Retrieve stored MKTU classes when the component mounts
    const storedClasses = getStoredMktuClasses();
    if (storedClasses && storedClasses.length > 0) {
      console.log("OperatorDashboard: Setting stored MKTU classes:", storedClasses);
      setSelectedClasses(storedClasses);
    }
  }, []);

  // Update MKTU classes in localStorage whenever they change
  useEffect(() => {
    // Store the selected classes whenever they change
    console.log("OperatorDashboard: selectedClasses changed:", selectedClasses);
    if (selectedClasses.length > 0) {
      storeMktuClasses(selectedClasses);
    }
  }, [selectedClasses]);
  ...
  const handleClassToggle = (classNum) => {
    setSelectedClasses(prevClasses => {
      ...
      if (prevClasses.includes(classNum)) {
        ...
        newClasses = prevClasses.filter(num => num !== classNum);
      } else {
        // Aks holda, uni qo'shish
        newClasses = [...prevClasses, classNum];
      }
      
      // Tanlangan sinflarni localStorage-ga saqlash
      console.log("handleClassToggle: Saving classes:", newClasses);
      storeMktuClasses(newClasses);
      
      return newClasses;
    });
  };

  // Tavsiya qilingan sinfni tanlash - yangilangan
  const selectRecommendedClass = (classNumber) => {
    setSelectedClasses(prevClasses => {
      if (prevClasses.includes(classNumber)) {
        return prevClasses;
      } else {
        const newClasses = [...prevClasses, classNumber];
        // Tanlangan sinflarni localStorage-ga saqlash
        console.log("selectRecommendedClass: Saving classes:", newClasses);
        storeMktuClasses(newClasses);
        return newClasses;
      }
    });
  ...
  // Barcha tavsiya qilingan sinflarni tanlash - yangilangan
  ...
  // Render class selection component - modify to ensure visibility of selected classes
  const renderClassSelection = () => {
    console.log("Current selected classes in renderClassSelection:", selectedClasses);
  ...
  // Create a function to handle power of attorney download
  const downloadPowerOfAttorney = async () => {
    ...
    try {
      ...
      alert('Ishonchnoma muvaffaqiyatli yaratildi!');
    } catch (error) {
