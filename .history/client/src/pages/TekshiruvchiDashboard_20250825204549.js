...
function TekshiruvchiDashboard() {
  ...
  const generatePowerOfAttorney = (task, type) => {
    ...
    // Debugging classes issue
    console.log('Task in generatePowerOfAttorney:', task);
    console.log('Task.classes before processing:', task.classes);
    
    // Tanlangan sinflarni tekshirish va olish - qayta yozilgan
    let classesString = '';
    
    if (task.classes && Array.isArray(task.classes) && task.classes.length > 0) {
      // Convert all elements to strings to ensure they display correctly
      classesString = task.classes.map(c => String(c)).join(', ');
      console.log('Classes from task:', classesString);
    } else {
      classesString = '[Sinf tanlanmagan]';
      console.log('No classes found in task');
    }
    
    console.log('Final classesString:', classesString);
    ...
    setPowerOfAttorneyData({
      ...
      fileName: fileName,
      classes: classesString // Store classes separately
    });
    setPowerOfAttorneyPreview(powerOfAttorneyContent);
    setShowPowerOfAttorney(true);
  };

  // Ishonchnomani Word formatida yuklab olish - Serverga saqlash qo'shildi
    ...
    try {
      ...
      const poa_res = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/save-power-of-attorney`, {
        ...
        body: JSON.stringify({
          ...
          classes: selectedTask.classes || [] // Ensure classes are included
        })
      });
      ...
      console.log('Power of attorney saved response:', await poa_res.json());
