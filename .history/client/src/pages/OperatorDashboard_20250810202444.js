...
const sendBrandForReview = async () => {
    ...
    try {
      ...
      // Xato bo'lsa, xato xabarini backenddan olishga harakat qiling
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.message || 'Brendni tekshiruvchiga yuborishda nomaʼlum xatolik');
      }
      ...
      alert(err.message); // Endi bu yerda backenddan kelgan xabar ko'rinadi
    } finally {
      setLoading(false);
    }
  };
