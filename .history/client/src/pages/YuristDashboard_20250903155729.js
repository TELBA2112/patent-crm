...
function YuristDashboard({ onLogout }) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setError('');
    try {
      let url = `${API_BASE}/api/jobs?status=to_lawyer,lawyer_processing,lawyer_completed`;
  ...
  // Bildirishnoma yuklash (notification letter)
  const onUploadNotification = async () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/upload-notification`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formData
        });
        if (!res.ok) throw new Error(await res.text());
        alert('Bildirishnoma yuklandi');
        fetchTasks();
      } catch (err) {
        alert('Xatolik: ' + err.message);
      }
  ...
  // Chatga xabar yuborish (tekshiruvchi bilan)
  const sendChat = async () => {
    if (!selectedTask || !chatText.trim()) return;
    try {
      const res = await fetch(`${API_BASE}/api/chat/${selectedTask._id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: (() => { const fd = new FormData(); fd.append('text', chatText); fd.append('toRole', 'tekshiruvchi'); return fd; })()
      });
      if (!res.ok) throw new Error(await res.text());
      setChatText('');
                  ...
                  {processingStep === 2 && (
                    <>
                      ...
                      <div className="notification-upload">
                        <h4>Bildirishnoma</h4>
                        <button onClick={onUploadNotification} className="step-action-btn">Bildirishnomani yuklash</button>
                      </div>
                    </>
                  )}
                  ...
                  {processingStep === 3 && (
                    <>
                      ...
                      <div className="chat-box">
                        <h4>💬 Tekshiruvchi bilan chat</h4>
                        <textarea value={chatText} onChange={(e) => setChatText(e.target.value)} placeholder="Xabar yo'zing..." className="text-input" />
                        <button onClick={sendChat} className="step-action-btn">Xabar yuborish</button>
                      </div>
                    </>
                  )}
                </div>
