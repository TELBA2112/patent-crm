...
function TekshiruvchiDashboard() {
  ...
  // Fetch stored MKTU classes on init and when needed
  useEffect(() => {
    // No need to set selectedClasses state since it's unused
    // You can still fetch or listen for MKTU class updates here if needed for side effects
    // (leave empty or remove this effect if not needed)
  }, []);
  ...
  const fetchDocumentsForReview = useCallback(async () => {
    setDocsLoading(true);
      // No need to update selectedClasses state since it's unused
