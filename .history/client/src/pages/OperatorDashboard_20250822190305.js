...
const sendBrandForReview = async () => {
    ...
    try {
      ...
      const data = await res.json();
      // selectedTask ni yangilash va modalni yopishdan oldin
      // vazifalar ro'yxatini qayta yuklash
      setSelectedTask(data);
      setShowModal(false); // Modalni yopish
      fetchTasks(selectedSection); // Ishlar ro'yxatini yangilash
      alert('Brend tekshiruvchiga muvaffaqiyatli yuborildi!'); // Muvaffaqiyat xabari
    } catch (err) {
      alert(err.message);
    } finally {
      setLoading(false);
