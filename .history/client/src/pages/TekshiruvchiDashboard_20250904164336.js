...
function TekshiruvchiDashboard() {
  ...
  const res = await apiFetch('/api/jobs?status=documents_submitted');
      ...
      const enrichedDocs = data.map(doc => {
        ...
        {currentSection === 'tugallangan' && (
          <div>
            <h2>🏁 Tugallangan ishlar</h2>
            {completedLoading && <p>Yuklanmoqda...</p>}
            {!completedLoading && completedTasks.length === 0 && <p>Hozircha ma'lumot yo'q</p>}
            {!completedLoading && completedTasks.length > 0 && (
              <ul className="task-list">
                {completedTasks.map((j) => (
                  <li key={j._id} className="task-item">
                    <div>
                      <strong>{j.brandName || '—'}</strong> — {j.clientName || ''} {j.clientSurname || ''}
                      <div className="meta">Status: {j.status}</div>
                    </div>
                    {Array.isArray(j.documents) && j.documents.some(d => d.type === 'certificate') && (
                      <a
                        href={(j.documents.find(d => d.type === 'certificate') || {}).path ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/${(j.documents.find(d => d.type === 'certificate') || {}).path}` : '#'}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Guvohnoma
  ...
  // Tugallangan ishlar (tekshiruvchi) - new view
  const fetchCompletedTasks = useCallback(async () => {
    setCompletedLoading(true);
    try {
      const res = await apiFetch('/api/jobs/views/tekshiruvchi/completed');
      if (!res.ok) throw new Error('Tugallangan ishlarni olishda xatolik');
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    if (currentSection === 'brend') {
      ...
      fetchTasks();
    } else if (currentSection === 'tugallangan') {
      fetchCompletedTasks();
  ...
  }, [token, currentSection, fetchTasks, fetchProfile, fetchDocumentsForReview, fetchPendingInvoices, fetchCompletedTasks]);
  ...
  const classes = Array.isArray(selectedTask.classes) ? [...new Set(selectedTask.classes.map(c => parseInt(c)).filter(n => !isNaN(n)))].sort((a,b)=>a-b) : [];
    return (
      <div className="modal">
      ...
      <p>Faoliyat turi (MKTU): {classes.length ? classes.join(',') + (classes.length>1 ? ' - sinflar' : ' - sinf') : '-'}</p>
              ...
              <div key={doc._id} className="document-item">
                <div className="document-info">
                  ...
                  <div style={{marginTop: 10}}>
                    <button
                      onClick={() => setOpenDocIds(prev => ({...prev, [doc._id]: !prev[doc._id]}))}
                      className="view-invoice-btn"
                    >
                      {openDocIds[doc._id] ? '◀️ Yopish' : '▶️ Batafsil'}
                    </button>
                  </div>

                  {openDocIds[doc._id] && doc.personType === 'yuridik' && doc.yuridikDocs && (
                    <div className="detailed-info-card yuridik-card compact-view">
                        ...
                        <div className="info-columns">
                          ...
                          <div className="info-column documents-preview">
                            <div className="document-previews-grid">
                              ...
                              {/* Show other documents in a compact grid */}
                              {doc.documents && doc.documents.map((document, idx) => (
                                <div key={idx} className="document-preview-item">
                                  <p className="document-title">{document.type || 'Hujjat'}</p>
                                  <div className="document-thumbnail-container">
                                    {document.path && (
                                      <button 
                  ...
                  {openDocIds[doc._id] && doc.personType === 'jismoniy' && doc.jismoniyDocs && (
                    <div className="detailed-info-card jismoniy-card compact-view">
                      <div className="detailed-info-header">
                      ...
                      <div className="detailed-info-body">
                        <div className="info-columns">
                          ...
                          <div className="info-column documents-preview">
                            <div className="document-images compact">
                              <div className="document-image-container compact">
                                <p className="image-title">Pasport:</p>
                                <div className="passport-thumbnails">
  ...
  const renderContent = () => {
    switch (currentSection) {
      ...
      case 'tolov_sorovi':
        return (
          <div className="dashboard-content">
            <h3>💰 To'lov so'rovi</h3>
            {profile ? (
              <WithdrawalRequest 
                userId={profile._id} 
                onRequestSubmitted={() => alert("To'lov so'rovi muvaffaqiyatli yuborildi!")}
              />
            ) : (
              <p>Profil yuklanmoqda...</p>
            )}
        ...
        return (
          <div className="dashboard-content">
            ...
            {completedTasksLoading ? (
              ...
              <p>Tugallangan ishlar mavjud emas.</p>
            ) : (
              <div className="task-list">
                {completedTasks.map(task => (
                  <div key={task._id} className="task-item completed-item">
                    <div className="task-details">
                      <h4>{task.brandName || 'Brend nomi mavjud emas'}</h4>
                      <p>Mijoz: {task.clientName} {task.clientSurname}</p>
                      <p>Status: <span className="status-completed">Tugallangan</span></p>
                      <p>ID: {task.jobId || task._id.slice(-5)}</p>
                      <HistoryBlock history={task.history} />
                      <p>Tugallangan sana: {new Date(task.completedAt || task.updatedAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))}
              </div>
