...
import { getStoredMktuClasses, storeMktuClasses } from '../utils/mktuData';
...
function YuristDashboard({ onLogout }) {
  ...
  // Add usage for setSelectedClasses
  useEffect(() => {
    // Listen for updates to MKTU classes from other dashboards
    const handleMktuClassesUpdated = (event) => {
      console.log('YuristDashboard: MKTU classes updated from event:', event.detail.classes);
  ...
  // Display selected classes in task details or other relevant parts of the UI
  const getClassesForTask = useCallback((task) => {
    if (task && task.classes && Array.isArray(task.classes) && task.classes.length > 0) {
      return task.classes;
    }
    
    // If not in task, use the selected classes
