...
function TekshiruvchiDashboard({ onLogout }) {
  ...
  const [brandStatus, setBrandStatus] = useState(''); // 'approved' yoki 'rejected'
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/review-brand`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="tekshiruvchi-dashboard-wrapper">
      <div className="dashboard-header">
        <h2>Tekshiruvchi Paneli</h2>
        {profile && <div className="profile-info">Xush kelibsiz, {profile.firstName}</div>}
      </div>

      <div className="tasks-section">
        <h3>Tekshirilishi kerak bo'lgan ishlar</h3>
        {tasksLoading ? (
          <p>Yuklanmoqda...</p>
        ) : tasks.length === 0 ? (
