...
function YuristDashboard({ onLogout }) {
  ...
  // (old, broken renderInvoiceForm block removed)
    ...
    // Formani oldindan to'ldirish: brand nomi va MKTU sinflarini task'dan yuklab qo'yamiz
  ...
  const onInvoiceFileSelected = async (e) => {
    ...
    // bir faylni qayta tanlay olish uchun inputni tozalaymiz
    e.target.value = '';
  };
  ...
  // To'g'ridan-to'g'ri To'lov bosqichiga o'tamiz
  setProcessingStep(2);
  ...
  const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/save-lawyer-docs`, {
      ...
      // UI ni yangilash: task ma'lumotlarini backenddan qaytgan qiymatlar bilan yangilaymiz
      if (data && data.job) {
        setSelectedTask(prev => ({ ...(prev || {}), ...data.job }));
      }
