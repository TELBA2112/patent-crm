...
function OperatorDashboard({ onLogout }) {
  ...
  const goToNextDocsStep = () => {
    ...
    if (personType === 'yuridik') {
      ...
      if (docsStep === 1 && !jismoniyDocs.passportImage) {
        return alert('Passport rasmini yuklang');
  ...
  const saveDocsAndSendToReview = async () => {
    ...
    try {
      ...
      alert('Hujjatlar muvaffaqiyatli yuborildi va tekshiruvchiga jo\'natildi!');
      setShowModal(false);
      fetchTasks(selectedSection);
    } catch (err) {
      ...
      case 1:
        return (
          <div className="docs-form-step">
            ...
            <div className="file-upload-container">
              <label className="file-upload-label">
                <span>Pasport rasmini yuklash (old va orqa)</span>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileUpload(e, 'jismoniy', 'passportImage')}
                  className="file-input"
                />
              </label>
              {jismoniyDocs.passportImage && (
                <div className="file-preview">
                  <img src={jismoniyDocs.passportImage} alt="Pasport" width="100" />
                </div>
              )}
            </div>
          </div>
