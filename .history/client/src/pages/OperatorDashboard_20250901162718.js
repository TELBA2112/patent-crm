...
function OperatorDashboard({ onLogout }) {
  ...
  // Brendni tekshiruvchiga yuborish - MKTU sinflarini to'g'ri saqlash bilan
  const sendBrandForReview = async () => {
    ...
    try {
      // First, update the MKTU classes separately
      const classesRes = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/update-mktu-classes`, {
        method: 'POST',
        headers: {
      ...
      if (!classesRes.ok) {
        ...
        console.error('MKTU sinflari yangilashda xatolik:', errorText);
        throw new Error(`MKTU sinflarini yangilashda xatolik: ${errorText}`);
      }
  ...
  // Hujjatlarni saqlash va tekshiruvchiga yuborish - MKTU sinflari to'g'ri saqlangan
  const saveDocsAndSendToReview = async () => {
    ...
    try {
      ...
      // First update the MKTU classes separately to ensure they're saved
      console.log('MKTU sinflari yangilanmoqda:', classesToSend);
      const classesRes = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/update-mktu-classes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      const res = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/submit-documents`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
