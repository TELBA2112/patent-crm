...
function OperatorDashboard({ onLogout }) {
  const [selectedSection, setSelectedSection] = useState('yangi'); // Tanlangan bo‘lim
  ...
  const [futureDate, setFutureDate] = useState(''); // Kelajakdagi sana
  const [brandName, setBrandName] = useState(''); // Brend nomi
  const [personType, setPersonType] = useState(''); // Shaxs turi
  ...
  const sendBrandForReview = async () => {
    ...
    setLoading(true);
    console.log('Brend yuborilmoqda:', { jobId: selectedTask._id, brandName }); // Debug log
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text(); // JSON bo‘lmasa, matn sifatida o‘qish
        console.error('Server javobi:', errorText);
        throw new Error(`Brendni yuborishda xatolik: ${res.status} - ${errorText}`);
      }
      const data = await res.json();
      console.log('Brend yuborildi:', data); // Debug log
