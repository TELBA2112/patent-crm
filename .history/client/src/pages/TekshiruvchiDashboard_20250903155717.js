...
function TekshiruvchiDashboard() {
  ...
  // Tugallangan ishlarni olish
  const fetchCompleted = useCallback(async () => {
    setCompletedLoading(true);
    try {
      const res = await apiFetch('/api/job-actions/completed');
  ...
  }, [token, currentSection, fetchTasks, fetchProfile, fetchDocumentsForReview, fetchPendingInvoices, fetchCompleted]);
  ...
  // Muammo bo'lsa chat ochish (yangi xabar)
  const openIssueChat = async (jobId, text) => {
    try {
      const res = await apiFetch(`/api/job-actions/${jobId}/open-issue-chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: text || 'Hujjatda xatolik, iltimos tekshirib qayta yuklang' })
      });
      if (!res.ok) throw new Error(await res.text());
      alert('Tekshiruvchi bilan chat ochildi');
    } catch (err) {
      alert('Xatolik: ' + err.message);
    }
  };
              ...
              <div key={doc._id} className="document-item">
                ...
                <div className="document-actions">
                  ...
                  <div className="action-buttons">
                    ...
                    <button 
                      onClick={() => openIssueChat(doc._id, feedbackComment)}
                      className="return-btn"
                    >
                      💬 Muammo bo'yicha chat
                    </button>
                  </div>
                </div>
              </div>
                ...
                {completedLoading ? (
                  ...
                  <div className="documents-list">
                    {completedJobs.map(job => (
                      <div key={job._id} className="document-item">
                        <h4>{job.brandName} — #{job._id.slice(-5)}</h4>
                        <p>Mijoz: {job.clientName} {job.clientSurname}</p>
                        <p>Telefon: {job.phone}</p>
                        <div>
                          {(job.documents || []).filter(d => d.type === 'certificate').map((c, i) => (
                            <a key={i} className="view-invoice-btn" href={`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/${String(c.path).replace(/^\/?/, '')}`} target="_blank" rel="noreferrer">
                              Guvohnomani yuklab olish #{i+1}
                            </a>
                          ))}
                        </div>
                      </div>
