...
function OperatorDashboard({ onLogout }) {
  ...
  const fetchTasks = useCallback(async (section) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/jobs?status=${status}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const sendBrandForReview = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/send-for-review`, {
        method: 'POST',
        headers: {
  ...
  const handleCallResultSubmit = async () => {
    if (callResult === 'boglandi') {
      ...
      try {
        await fetch(`${API_BASE}/api/jobs/${selectedTask._id}`, {
          method: 'PUT',
  ...
  const saveDocsAndSendToReview = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/submit-documents`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const generatePowerOfAttorney = async (task, type) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/power-of-attorney/${task._id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const downloadPowerOfAttorney = async (format = 'docx') => {
    ...
    try {
      ...
      const saveRes = await fetch(`${API_BASE}/api/jobs/${selectedTask._id}/power-of-attorney`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      if (format === 'pdf') {
        ...
        const pdfRes = await fetch(`${API_BASE}/api/jobs/${selectedTask._id}/power-of-attorney-pdf`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
