...
function TekshiruvchiDashboard() {
  ...
  // Ishonchnoma yaratish funksiyasi - avtomatik to'ldirish bilan - MKTU sinflarni to'g'ri saqlash bilan
  const generatePowerOfAttorney = async (task, type) => {
    ...
    try {
      ...
      if (task.classes && Array.isArray(task.classes)) {
        ...
        const storedClasses = getStoredMktuClasses();
        if (storedClasses && storedClasses.length > 0) {
          classNumbers = storedClasses.map(cls => parseInt(cls)).filter(cls => !isNaN(cls));
        }
      }
      
      console.log('Extracted class numbers:', classNumbers);
      
      // First update the MKTU classes in the database
      if (classNumbers.length > 0) {
        const classesRes = await fetch(`http://localhost:5000/api/job-actions/${task._id}/update-mktu-classes`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
        ...
        if (!classesRes.ok) {
          ...
          console.warn('MKTU sinflarini yangilashda xatolik:', errorText);
          // Continue anyway as this is not critical
        } else {
          console.log('MKTU sinflari muvaffaqiyatli yangilandi');
        }
      }
      
      // Then continue with the regular power of attorney generation
      const now = new Date();
  ...
  // Ishonchnomani Word formatida yuklab olish va serverga saqlash
