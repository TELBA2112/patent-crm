...
function OperatorDashboard({ onLogout }) {
  ...
  const fetchTasks = useCallback(async (section) => {
    ...
    switch(section) {
      case 'yangi':
        status = 'yangi,returned_to_operator'; // Yangi va qaytarilgan ishlar
        break;
      case 'jarayonda':
        status = 'bajarilmoqda,brand_in_review,documents_pending';
        break;
      case 'tugatilgan':
    ...
    try {
      ...
      // Qaytarilgan ishlarni alohida ko'rsatish
      const tasks = data.map(task => {
        if (task.status === 'returned_to_operator') {
          // Rad etilgan va operatorga qaytgan ishlarni alohida belgilash
    ...
    <div className="tasks-list-modern">
      ...
      {tasksLoading ? <p>Yuklanmoqda...</p> : tasks.length === 0 ? <p>Ishlar topilmadi</p> : (
        <div className="task-cards-modern">
          {tasks.map(task => (
            <div key={task._id} 
                 className={`task-card-modern ${task.isReturned ? 'returned-task' : ''}`} 
