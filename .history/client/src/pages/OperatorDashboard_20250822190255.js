...
function OperatorDashboard({ onLogout }) {
  ...
  const sendBrandForReview = async () => {
    ...
    try {
      ...
      setShowModal(false);
      fetchTasks(selectedSection);
      alert('Brend administratorga tekshiruvga yuborildi!');
    } catch (err) {
      alert(err.message);
    } finally {
      setLoading(false);
      ...
      case 3:
        return (
          <div className="step-box">
            ...
            <p>Mijozning brend nomini kiriting. U administratorga yuboriladi.</p>
            <input
              type="text"
            ...
            />
            <button onClick={sendBrandForReview} className="btn-primary" disabled={!brandName || loading}>Administratorga yuborish</button>
          </div>
        );
      case 4:
        return (
