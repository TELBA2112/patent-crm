...
function Tasks({ token, refreshTrigger }) {
  ...
  const archiveJob = async (id) => {
    if (!window.confirm('Ushbu ishni arxivga o\'tkazishni tasdiqlaysizmi?')) return;
    try {
      const res = await apiFetch(`/api/job-actions/${id}/archive`, { method: 'POST' });
              ...
              <>
                <tr key={task._id}>
                  ...
                  <td>
                    ...
                    {task.status === 'lawyer_completed' && (
                      <button onClick={() => archiveJob(task._id)}>Arxivlash</button>
                    )}
                  </td>
                </tr>
