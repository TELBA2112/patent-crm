...
const [form, setForm] = useState({
  cl                        {openId === task._id ? 'Tarixni yopish' : 'Tarixni ko\'rish'}entName: '',
  clientSurname: '',
  phone: '',
  brandName: '',
  status: 'yangi',
...
<td>
  ...
  <button
    onClick={() => viewJobDetails(task)}
    style={{ marginLeft: 8, background: '#2e86de' }}
  >
    Ma'lumotlar
  </button>
  ...
  {Array.isArray(task.history) && task.history.length > 0 && (
    <button
      onClick={() =>
        setOpenId((prev) => (prev === task._id ? null : task._id))
      }
      style={{ marginLeft: 8 }}
    >
  ...
  const fetchJobDocuments = async (jobId) => {
    if (!jobId) return;
    try {
      const res = await apiFetch(`/api/job-actions/${jobId}/documents`);
      if (!res.ok) {
        throw new Error('Hujjatlarni olishda xatolik');
      }
      const data = await res.json();
      setJobDocuments(data.documents || []);
    } catch (err) {
      console.error('Hujjatlar olishda xatolik:', err);
      setJobDocuments([]);
    }
  };
  
  // Job payouts olish
  const fetchJobPayouts = async (jobId) => {
    if (!jobId) return;
    try {
      const res = await apiFetch(`/api/job-actions/${jobId}/payouts`);
      if (!res.ok) {
        if (res.status === 404) {
          // Agar payouts mavjud bo'lmasa bo'sh ro'yxat qaytaramiz
          setPayouts([]);
          return;
        }
      ...
      console.error('To\'lovlar olishda xatolik:', err);
      setPayouts([]);
    }
  };
  ...
  // To'lov yaratish funksiyasi
  const createPayout = async (jobId) => {
    if (!payoutForm.role || !payoutForm.amount) {
      alert("Xodim roli va to'lov miqdorini kiriting");
      return;
    }
    ...
    try {
      ...
      setPayoutForm({ role: 'operator', amount: '', comment: '' });
      await fetchJobPayouts(jobId);
    } catch (err) {
      alert("Xatolik: " + err.message);
    }
  };
  
  // Tanlangan ishni batafsil ko\'rish
  const viewJobDetails = async (job) => {
    setSelectedJob(job);
    setShowJobDetails(true);
    await fetchJobDocuments(job._id);
    await fetchJobPayouts(job._id);
  };
