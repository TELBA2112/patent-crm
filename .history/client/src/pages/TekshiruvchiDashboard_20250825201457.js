...
function TekshiruvchiDashboard() {
  ...
  const generatePowerOfAttorney = (task, type) => {
    ...
    // MKTU sinflarini tekshirib olish - to'g'rilangan
    let classesString = '';
    ...
    console.log("MKTU classes for power of attorney:", task.classes, classesString);
    ...
    if (type === 'jismoniy') {
      ...
      // Faqat JSHSHR tekshiruvchi tomonidan kiritiladi
  ...
  const downloadPowerOfAttorney = async () => {
    ...
    try {
      ...
      await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/save-power-of-attorney`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        ...
        if (!res.ok) throw new Error('Ishonchnomani serverga saqlashda xatolik');
        return res.json();
      }).then(data => {
        console.log('Ishonchnoma saqlandi:', data);
      });
