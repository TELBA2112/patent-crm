...
function YuristDashboard({ onLogout }) {
  ...
  const notificationInputRef = useRef(null); // Add ref for notification input
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setError('');
    try {
      // Optimize API calls to ensure we get data properly
      let url;
      ...
      // Use apiFetch instead of fetch for consistent headers
      const res = await apiFetch(url);
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ message: 'Ishlarni olishda xatolik' }));
  ...
  // Bildirishnoma tanlash va yuklash
  const onClickNotificationUpload = () => notificationInputRef.current && notificationInputRef.current.click();
  const onNotificationFileSelected = async (e) => {
    const file = e.target.files && e.target.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/') && file.type !== 'application/pdf') {
      alert('Faqat PDF yoki rasm fayllarini yuklash mumkin');
    ...
    await uploadNotification(file);
    e.target.value = '';
  };
  
  // Bildirishnoma faylini yuklash
  const uploadNotification = async (file) => {
    if (!selectedTask || !file) return;
    try {
      const formData = new FormData();
      formData.append('notice', file);

      const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/upload-notice`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });
      if (!res.ok) throw new Error(await res.text());
      alert('Bildirishnoma muvaffaqiyatli yuklandi!');
      fetchTasks();
    } catch (err) {
      alert('Xatolik: ' + err.message);
    }
  ...
  const handleAcceptTask = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/job-actions/${selectedTask._id}/accept-by-lawyer`, {
        ...
        body: JSON.stringify({ comment: comment || 'Yurist ishni qabul qildi' })
      });
      ...
      // Get the updated job data from the response
      const responseData = await res.json().catch(() => ({ status: 'lawyer_processing' }));
      ...
      setSelectedTask(prev => ({
        ...prev,
        status: responseData.status || 'lawyer_processing'
      }));
      setProcessingStep(2); // Move to next step automatically
      
      // Refresh the task list immediately
      fetchTasks();
      
      // Don't close modal yet - let the user continue to the next step
    } catch (err) {
      console.error('Ish qabul qilishda xatolik:', err);
      alert('Xatolik: ' + (err.message || 'Server bilan aloqa o\'rnatishda xatolik'));
    }
  };
  ...
  const saveInvoiceAndNotifyClient = async (file) => {
    ...
    try {
      ...
      // Ask user to upload notification
      const uploadNotice = window.confirm("To'lov hisobi yuborildi. Bildirishnoma ham yuklashni istaysizmi?");
      if (uploadNotice) {
        // Trigger notification file selection
        notificationInputRef.current?.click();
      } else {
        setProcessingStep(2);
        fetchTasks();
      }
  ...
  // Handle the upload of notification document
  ...
  // Bildirishnoma yuklash formasi
  const renderNotificationUploadForm = () => {
    return (
      <div className="notification-upload">
        <h4>Bildirishnoma</h4>
        <button onClick={onClickNotificationUpload} className="upload-notification-btn">
          Bildirishnoma faylini yuklash (PDF yoki rasm)
        </button>
        <input
          type="file"
          ref={notificationInputRef}
          onChange={onNotificationFileSelected}
          accept="application/pdf,image/*"
          style={{ display: 'none' }}
        />
      </div>
              ...
              <div className="processing-steps-container">
                <div className="steps-indicator">
                  ...
                  <div className={`step ${processingStep >= 3 ? 'active' : ''}`} onClick={() => setProcessingStep(3)}>
                    ...
                    <div className="step-name">Bildirishnoma</div>
                  </div>
                  <div className="step-connector"></div>
                  <div className={`step ${processingStep >= 4 ? 'active' : ''}`} onClick={() => setProcessingStep(4)}>
  ...
  // Sidebar - hisobot bo'limi va to'lov so'rovi qo'shildi
  const renderSidebar = () => (
    <div className="yurist-sidebar">
      ...
      <div className="sidebar-links">
        ...
        <div 
          className={`sidebar-link ${selectedSection === 'tolov' ? 'active' : ''}`} 
          onClick={() => setSelectedSection('tolov')}
        >
          💰 To'lov so'rovi
        </div>
        <div 
