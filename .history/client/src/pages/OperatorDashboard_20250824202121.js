...
function OperatorDashboard({ onLogout }) {
      ...
      case 4:
        return (
          <div className="step-box">
            ...
            {/* Yuristga yuborish qismi */}
            <div className="lawyer-section">
              <h4>Yuristga yuborish</h4>
              <p>Agar mijoz hujjatlarni topshirgan bo'lsa, ishni yuristga yuboring:</p>
              <textarea
                placeholder="Yuristga xabar (ixtiyoriy)"
                value={lawyerComment}
                onChange={e => setLawyerComment(e.target.value)}
                className="lawyer-comment"
              ></textarea>
              <button
                onClick={sendToLawyer}
                className="btn-send-to-lawyer"
                disabled={loading}
              >
                Yuristga yuborish
              </button>
  ...
  // Yuristga yuborish uchun o'zgaruvchilar
  const [lawyerComment, setLawyerComment] = useState('');

  // Yuristga yuborish funksiyasi
  const sendToLawyer = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/send-to-lawyer`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ comment: lawyerComment })
      });
      
      if (!res.ok) {
        const errorText = await res.text();
        console.error('Server javobi:', errorText);
        throw new Error(`Yuborishda xatolik: ${res.status} - ${errorText}`);
      }
      
      alert('Ish yuristga muvaffaqiyatli yuborildi!');
      setShowModal(false);
      fetchTasks(selectedSection);
    } catch (err) {
      console.error('Frontend xatosi:', err);
