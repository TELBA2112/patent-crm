...
function OperatorDashboard({ onLogout }) {
  ...
  const saveDocsAndSendToReview = async () => {
    ...
    try {
      ...
      // Ensure classes are included in the upload
      const classesToSend = selectedTask.classes || selectedClasses;
      
      console.log('Yuborilayotgan ma\'lumotlar:', {
        personType,
        classes: classesToSend, // Include classes in the log
        ...docsData
      });
      ...
      const res = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/upload-docs`, {
        ...
        body: JSON.stringify({
          personType,
          classes: classesToSend, // Include classes in the request
          ...docsData
        })
      });
  ...
  const renderModalContent = () => {
    ...
    console.log('Selected task classes:', selectedTask?.classes);
      ...
      case 4:
        return (
          <div className="step-box">
            ...
            {/* Display selected MKTU classes */}
            <div className="selected-mktu-classes">
              <h4>📋 Tanlangan MKTU sinflari:</h4>
              {selectedClasses && selectedClasses.length > 0 ? (
                <div className="classes-list">
                  {selectedClasses.sort((a, b) => a - b).map(classNum => (
                    <span key={classNum} className="class-badge">{classNum}</span>
                  ))}
                </div>
              ) : (
                <p className="no-classes-message">MKTU sinflari tanlanmagan</p>
              )}
            </div>
