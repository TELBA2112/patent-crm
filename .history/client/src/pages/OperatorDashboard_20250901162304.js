...
function OperatorDashboard({ onLogout }) {
  ...
  const sendBrandForReview = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/job-actions/${selectedTask._id}/send-for-review`, {
        ...
        body: JSON.stringify({ 
          brandName, 
          classes: selectedClasses.map(classNum => parseInt(classNum)) // Ensure all classes are numbers
        })
      });
  ...
  const handleTaskClick = async (task) => {
    ...
    try {
      // Taskdagi MKTU sinflarini olish - API orqali
      const res = await fetch(`http://localhost:5000/api/jobs/${task._id}/mktu-classes`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (res.ok) {
        ...
        if (data.classes && Array.isArray(data.classes) && data.classes.length > 0) {
          console.log('API orqali olingan MKTU sinflari:', data.classes);
          const classNumbers = normalizeMktuClasses(data.classes);
          setSelectedClasses(classNumbers);
        } else {
          // Fall back to what's in the task if API returns empty
        ...
        // Fall back to what's in the task if API fails
        if (task.classes && Array.isArray(task.classes)) {
          const classNumbers = normalizeMktuClasses(task.classes);
          console.log('Task dan olingan MKTU sinflari:', classNumbers);
          setSelectedClasses(classNumbers);
      ...
      console.error('MKTU sinflarini olishda xatolik:', err);
      // Fall back to what's in the task if API throws error
      if (task.classes && Array.isArray(task.classes)) {
        const classNumbers = normalizeMktuClasses(task.classes);
        console.log('Task dan olingan MKTU sinflari:', classNumbers);
        setSelectedClasses(classNumbers);
  ...
  // Hujjatlarni saqlash va tekshiruvchiga yuborish - yangilangan to properly include MKTU classes
  const saveDocsAndSendToReview = async () => {
    if (!selectedTask?._id) {
      return alert('Ish tanlanmagan');
    }

    // MKTU sinflar tekshirilishi va qo'shilishi
    ...
    // Ensure we have MKTU classes - this is a critical check
    ...
    try {
      ...
      // First save the MKTU classes separately to ensure they're saved properly
      await saveMktuClassesToDatabase(selectedTask._id, classesToSend);
      ...
      const res = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/upload-docs`, {
        ...
        body: JSON.stringify({
          personType,
          classes: classesToSend.map(num => parseInt(num)), // Ensure they're numbers
          ...docsData
        })
      });
  ...
  const generatePowerOfAttorney = async (task, type) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/power-of-attorney/${task._id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const downloadPowerOfAttorney = async (format = 'docx') => {
    ...
    try {
      ...
      const saveRes = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/power-of-attorney`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      if (format === 'pdf') {
        ...
        const pdfRes = await fetch(`http://localhost:5000/api/jobs/${selectedTask._id}/power-of-attorney-pdf`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const renderClassSelection = () => {
    return (
      <div className="class-selection">
        ...
        {/* MKTU sinflarini saqlash tugmasi - qo'shimcha */}
        {selectedTask && selectedClasses.length > 0 && (
          <div className="save-classes-container">
            <button 
              onClick={() => saveMktuClassesToDatabase(selectedTask._id, selectedClasses)} 
              className="save-classes-btn"
              disabled={loading}
            >
              {loading ? 'Saqlanmoqda...' : 'MKTU sinflarini saqlash'}
            </button>
          </div>
        )}
      </div>
  ...
  // MKTU sinfini tanlash yoki bekor qilish funksiyasi - yangilangan
  ...
  // MKTU sinflarini API orqali bazaga saqlash - yangi funksiya
  const saveMktuClassesToDatabase = async (jobId, classNumbers) => {
    if (!jobId || !classNumbers || classNumbers.length === 0) {
      alert("Ish yoki MKTU sinflari tanlanmagan!");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/jobs/${jobId}/mktu-classes`, {
        ...
        body: JSON.stringify({
          classes: classNumbers.map(num => parseInt(num)) // Ensure they're numbers
        })
      });
      
      if (!res.ok) {
        const errorText = await res.text();
        throw new Error(`MKTU sinflarini saqlashda xatolik: ${res.status} - ${errorText}`);
      }
      
      const data = await res.json();
      console.log('MKTU sinflari saqlandi:', data);
      
      // Success message
      alert('MKTU sinflari muvaffaqiyatli saqlandi!');
      
      // Refresh the task data if needed
      if (selectedSection === 'jarayonda' || selectedSection === 'yangi') {
        fetchTasks(selectedSection);
      }
  ...
  // Brendni tekshiruvchiga yuborish - yangilangan to properly send MKTU classes
  const sendBrandForReview = async () => {
    if (!brandName) {
      alert('Iltimos, brend nomini kiriting.');
      return;
      ...
      if (type === 'jismoniy') {
        ...
        powerOfAttorneyContent = `
          <div style="padding: 20px; font-family: 'Times New Roman', serif; line-height: 1.5;">
            ...
            <p style="text-align: justify; margin-bottom: 15px;">
              Ишончнома, Турдиалиев Мухаммад Али Пулатжон угли (ЎзР Адлия вазирлигидан ҳуқуқий маслаҳат кўрсатувчи тижорий ташкилот сифатида 98-сонли рақам билан реестрдан ўтган Буюртмачи ${task.clientName} ${task.clientSurname} номидан Адлия вазирлиги ҳузуридаги "Интелектуал Мулк Маркази" давлат муассасасига ариза, давлат божларини тўлашда, сўровномаларга жавоб беришда, билдиришнома, Талабнома ва бошқа ҳужжатларни товар белгисини рўйхатдан ўтказиш учун топширишга ваколат ва ҳуқуқ берил