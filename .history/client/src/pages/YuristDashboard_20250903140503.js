...
function YuristDashboard({ onLogout }) {
  ...
  const handleTaskClick = (task) => {
    ...
    setShowModal(true);
    setComment('');
    // Auto step based on current job state (ensure payment form appears in processing)
    ...
    if (task.status === 'lawyer_completed' || isPaid) {
      setProcessingStep(3);
    } else if (hasPending || hasReceipt) {
      setProcessingStep(2);
    } else if (task.status === 'lawyer_processing') {
      setProcessingStep(2);
    } else if (task.status === 'to_lawyer') {
      setProcessingStep(1);
    } else {
      setProcessingStep(1);
    }
